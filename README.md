...
device), or play the one **bundled** game — `guess.prg`, an original C64 BASIC
game written for this project and released to the public domain
(`public/roms/`, generated by a small BASIC tokenizer). No commercial or
third-party ROMs are distributed here: "free to play" is not "free to
redistribute". The build is multi-page (Vite `rollupOptions.input`).
