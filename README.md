...
## C64 Games (`games.html`)

A second page runs actual Commodore 64 games in the browser via
[EmulatorJS](https://emulatorjs.org) (VICE `c64` core, loaded from their CDN).
**Load your own** `.d64`/`.prg`/`.t64`/`.crt` file (it never leaves your
device), or play the **bundled** original games in `public/roms/` — *Spy Chase*
(steer/shoot dodger) and *Guess* — both written for this project and released
to the public domain, generated from C64 BASIC by the tokenizer in
`scripts/tokenize.py`. No commercial or third-party ROMs are distributed here:
"free to play" is not "free to redistribute". The build is multi-page (Vite
`rollupOptions.input`).
